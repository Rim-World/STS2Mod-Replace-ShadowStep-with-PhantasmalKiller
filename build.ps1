...
$modOut = Join-Path $game 'mods\PhantomKillerMod'

Write-Host "==> Build .NET assembly"
dotnet build (Join-Path $root 'PhantomKillerMod.csproj') -c Debug
if ($LASTEXITCODE -ne 0) { throw 'dotnet build failed' }

$dll = Join-Path $root '.godot\mono\temp\bin\Debug\PhantomKillerMod.dll'
$pdb = Join-Path $root '.godot\mono\temp\bin\Debug\PhantomKillerMod.pdb'
...
try {
    ...
    $pck = Join-Path $modOut 'PhantomKillerMod.pck'
...
Copy-Item $dll (Join-Path $modOut 'PhantomKillerMod.dll') -Force
if (Test-Path $pdb) { Copy-Item $pdb (Join-Path $modOut 'PhantomKillerMod.pdb') -Force }
Copy-Item (Join-Path $root 'PhantomKillerMod.json') (Join-Path $modOut 'PhantomKillerMod.json') -Force
